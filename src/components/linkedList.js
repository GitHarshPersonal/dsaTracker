import React, {useState, useEffect} from 'react';
import './ques.css';
let questions = require('../source/linkedList.json');

export default function LinkedList() {

      let initQues;
      if(localStorage.getItem("linkedData") === null)
      {
        localStorage.setItem("linkedData", JSON.stringify(questions));
      }
      initQues = JSON.parse(localStorage.getItem("linkedData"));
      
      const [linkedData, setlinkedData ] = useState(initQues);
      
      useEffect(()=>{
        localStorage.setItem("linkedData", JSON.stringify(linkedData));
      }, [linkedData])
      

      let i = 0;
      linkedData.map(k => {
        k.id = i;
        i = i+1;
      })

      const toggle = (id)=>{

        let mapped = linkedData.map(e => {
          return e.id === Number(id) ? {...e, Done: !e.Done } : {...e};
        });
        setlinkedData(mapped);

      }

      let numDone=0,num;

      num = linkedData.length;
      
      for(let i=0;i<linkedData.length;i++)
      {
        if(linkedData[i].Done === true) numDone++;
      }

      let perc = 100*(numDone/num);

      console.log(numDone);

      return (
        <div className="question">
          <div className="stats">
            <h1>Done {numDone} out of {num}</h1>
            <h1>{perc.toFixed(2)}% Done</h1>
          </div>
          {linkedData.map((ques) => (
            <div className="ques" key={ques.id + ques['Problem: ']}>
              <a href={ques.URL} target="_blank">
              <span className={ques.Done ? "quesName Done" : "quesName"}>{ques['Problem: ']}</span>
              </a>
              <button id='done-btn' onClick={()=>{toggle(ques.id)}}>{!ques.Done? "Done" : "Undo Done"}</button>
            </div>
          ))}
        </div>
      );
}